...
async function startQuiz(){ const data=await $fetch<{questions:QuizQuestion[]}>('/api/quiz/next?limit=20'); quizQuestions.value=data.questions; quizIndex.value=0; quizScore.value=0; selectedOptionId.value=null; answered.value=false; answerResult.value=null; showTranslation.value=false }
...
async function dontKnow(){ if(!quizCurrent.value||answered.value) return; const res=await $fetch<{correct:boolean;correctDefinition?:string|null}>('/api/quiz/answer',{method:'POST',body:{wordId:quizCurrent.value.wordId,selectedOptionId:quizCurrent.value.wordId, forceWrong:true}}); answered.value=true; answerResult.value=res; showTranslation.value = true; await loadStats() }
function nextQuestion(){ if(quizIndex.value<quizQuestions.value.length-1){ quizIndex.value++; selectedOptionId.value=null; answered.value=false; answerResult.value=null; showTranslation.value=false }}
  ...
  <main class="wrap">
    ...
    <section class="card">
      ...
      <div v-if="quizCurrent" class="current">
        <div class="term">{{ quizCurrent.prompt }} <button class="ghost" @click="showTranslation = !showTranslation">Translate</button></div>
        <div v-if="showTranslation && quizCurrent.translationRu" class="translation">{{ quizCurrent.translationRu }}</div>
...
.quiz-top{display:flex;gap:.8rem;align-items:center;flex-wrap:wrap}.current{padding:1rem;border:1px dashed #3d4468;border-radius:10px}.term{font-size:1.5rem;font-weight:700;display:flex;gap:.6rem;align-items:center}
